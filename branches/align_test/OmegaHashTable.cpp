...
/**********************************************************************************************************************
	Default Constructor
**********************************************************************************************************************/
OmegaHashTable::OmegaHashTable(void)
{
...
**********************************************************************************************************************/
bool OmegaHashTable::insertDataset(QueryDataset* d, UINT64 minOverlapLength)
{
	CLOCKSTART;
	MEMORYSTART;
	dataSet=d;
...
**********************************************************************************************************************/
bool OmegaHashTable::hashRead(QueryRead *read)
{
...
**********************************************************************************************************************/
void OmegaHashTable::setHashTableSize(UINT64 size)
{
...
**********************************************************************************************************************/
UINT64 OmegaHashTable::hashFunction(string subString)
{
...
**********************************************************************************************************************/
bool OmegaHashTable::insertIntoTable(QueryRead *read, string subString, UINT64 orientation)
{
	UINT64 ID = read->getIdentifier() | (orientation << 62); 	// Most significant two bits are used to store the orientation of the string in the read.
	...
	while(!hashTable->at(index)->empty())
	{
        ...
        // Ted: clear cases
		string str = (orient == 0 || orient == 1) ? dataSet->getReadFromID(readNumber)->getSequence() : dataSet->getReadFromID(readNumber)->reverseComplement();
	...
	{
		cout << currentCollision << " collisions for read " << read->getIdentifier() << " " << subString << " " << orientation << endl;
	}
	return true;
}
...
**********************************************************************************************************************/
vector<UINT64> * OmegaHashTable::getListOfReads(string subString)
{
